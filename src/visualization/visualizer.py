...
class MissionVisualizer:
    ...
    def visualize(
        ...
        fig.suptitle(
            "UAV Agricultural Mission Planning", fontsize=14, fontweight="bold"
        )
    ...
    def draw_panel(
        ...
        for i, zone_result in enumerate(result.zones):
            ...
            if zone_result.path:
                ...
                for depart, home, resume in _rth_legs(
                    zone_result.path, drone.start_position
                ):
                    ax.plot(
                        [depart[0], home[0]],
                        [depart[1], home[1]],
    ...
    def save_bcd_decomposition_figure(
        ...
        fx, fy = self.field_polygon.exterior.xy
        ax.fill(fx, fy, alpha=0.08, color="gray", zorder=0)
        ax.plot(fx, fy, "k-", linewidth=2.0, zorder=1)
        for interior in self.field_polygon.interiors:
            hx, hy = interior.xy
            ax.fill(hx, hy, color="white", zorder=2)
            ax.plot(hx, hy, "k--", linewidth=1.4, zorder=3)
        ...
        for i, zone_result in enumerate(result.zones):
            ...
            cell_label0 = 0
            for part in _zone_polygon_parts(zone_result.zone_polygon):
                if part.is_empty or part.area <= 0.0:
                    continue
                _, cells, _safe = decompose_field(part, swath, angle_deg)
                ...
                cell_label0 += len(cells)

        ax.set_title(
            "BCD (per zone, safe area)\n" f"Total cells: {total_cells}",
            fontsize=11,
            fontweight="bold",
        )
        ax.set_xlabel("X (m)")
    ...
    def simulate(
            ...
            if zone_result.path:
                ...
                for pt in zone_result.path:
                    if _is_nan_point(pt):
                        if seg_x:
                            ax.plot(
                                seg_x, seg_y, "-", color=color, linewidth=0.7, alpha=0.3
                            )
                            seg_x, seg_y = [], []
                    else:
                        seg_x.append(pt[0])
        ...
        for i, drone in enumerate(self.drones):
            ...
            (trail,) = ax.plot(
                [], [], "-", color=color, linewidth=1.8, alpha=0.85, zorder=5
            )
